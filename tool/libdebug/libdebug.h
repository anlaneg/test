...
{
	DEBUG_LEVEL = 1, INFO_LEVEL, LOG_LEVEL, WARN_LEVEL, ERROR_LEVEL, FAULT_LEVEL
};
 ...
 * @return >=0 获取成功
 */
static inline int32_t debug_get_module_name_index(
		const char* debug_module_names[], const char* module_name)
{
	int32_t idx;
 ...
 * @param[in] level 日志级别
 */
static inline void debug_set_module_log_level(enum lib_debug_level level)
{
	g_debug_level = level;
}

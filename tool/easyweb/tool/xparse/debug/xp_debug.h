...
/*定义模块数目*/
#define XP_MODULE_COUNT (9ULL)

/*定义相应模块的对外标记*/
#define CHECK (1ULL << 0) /*语法检查模块*/
#define GEN (1ULL << 1) /*生成模块*/
#define PARSE (1ULL << 2) /*解析模块*/
#define DEBUG (1ULL << 3) /*debug模块*/
#define FRAME (1ULL << 4) /*frame模块*/
#define PLUGIN (1ULL << 5) /*插件模块*/
#define SYNTAX (1ULL << 6) /*语法模块*/
#define SYNTAX_CHECK (1ULL << 7) /*语法检查*/
#define SYNTAX_TREE (1ULL << 8) /*语法树*/


/*debug模块名称*/
extern const char* g_debug_module_name[XP_MODULE_COUNT + 1];

/**
 * 开启模块函数
 * @param[in] module_name 要开启的模块名称,例如"CHECK"
 * @return 0 开启成功
...
/**
 * 关闭模块函数
 * @param[in] module_name 要关闭的模块名称，例如"CHECK"
 * @return 0 关闭成功
...
{
	g_debug_module_flag = (1ULL << (XP_MODULE_COUNT + 1)) -1;
}


/**
 * 模块模块是否被开启
 * @param[in] module_name 模块名称，例如"CHECK"
 * @return 0 此模块未开启
 * @return !0 此模块已开启
 */

...
#c文件生成.o文件
$(OUT_DIR)/%.o:%.c
	$(call amf_command_execute ,$(CC) $(C_COMPLIER_FLAGS) $(INCLUDE_PATH) -c -o $@ $^)

#c++文件生成.oo文件
$(OUT_DIR)/%.oo:%.cpp
	$(call amf_command_execute ,$(CC) $(CPLUS_COMPLIER_FLAGS) $(INCLUDE_PATH) -c -o $@ $^)
...
__mk_target__: $(SRCS_OBJECT) $(CPLUS_SRCS_OBJECT) $(SUB_MODULE_OBJECT)
	$(CC) $(C_COMPLIER_FLAGS) -fPIC -shared -Wl,-soname,$(shell basename $(TARGET_NAME)) -o $(TARGET_NAME) $^
endif

ifeq ($(strip $(TARGET_TYPE)),obj)
__mk_target__: $(SRCS_OBJECT) $(CPLUS_SRCS_OBJECT)  $(SUB_MODULE_OBJECT) 
	$(call amf_command_execute,$(CC) $(C_COMPLIER_FLAGS) -nostdlib -r -o ../$(OUT_DIR)/$(SUB_MODULE_PREFIX)$(shell basename $(CURDIR)).o $^)
endif
...
debug_amf:
	...
	$(call amf_debug_log,  "INCLUDE_PATH=$(INCLUDE_PATH)")

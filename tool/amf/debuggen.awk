...
{
	printf("cat %s/template_debug.h | awk '{sub(\"##FILE_NAME_UPPER##\",\"%s\"); sub(\"##DEBUG_MODULE_COUNT##\",\"%d\");sub(\"##DEBUG_MODULE_MACRO_FLAG##\",\"%s\");sub(\"##DEBUG_MODULE_EXAMPLE##\",\"%s\"); print $0 }' > %s/%s_debug.h;",debug_awk_path,file_name_toupper,count,file_module_macro,file_module_example_name,out_path,file_name);
}	
			
	
function gen_cfile()
{
	printf("cat %s/template_debug.c | awk '{sub(\"##FILE_NAME##\",\"%s\"); sub(\"##FILE_NAME_UPPER##\",\"%s\");sub(\"##DEBUG_MODULE_NAME##\",\"%s\"); print $0; }' > %s/%s_debug.c;",debug_awk_path,file_name,file_name_toupper,file_module_names,out_path,file_name);
}
